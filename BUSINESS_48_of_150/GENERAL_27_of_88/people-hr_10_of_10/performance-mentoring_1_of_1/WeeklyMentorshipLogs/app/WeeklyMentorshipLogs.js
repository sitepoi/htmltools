...
(function () {
	...
	var CFG = {
		...
		weekly: true,
		roleLabel: "Mentor",
		streamNoun: "Mentorship",
		cadenceLabel: "Weekly Mentorship",
		recordNoun: "mentorship record",
		reportTitleDefault: "Weekly Mentorship Record",
		pdfSectionTitle: "Weekly Mentorship Records",
		pdfColor: [176, 106, 44],
		excelSheet: "Mentorship",
		fileTag: "mentorship",
		unique: true,
		emptyTitle: "No mentorship records yet",
		emptyBody: "Add a weekly reflection \u2014 skills covered, progress, challenges and goals for the week ahead.",
		dateField: { id: "week", label: "Week" },
		authorField: { id: "mentor", label: "Mentor" },
		hasRating: false,
		hasHours: false,
		fields: [
			{ id: "topics", label: "Topics & skills covered", ph: "What was discussed or developed this week?" },
			{ id: "progress", label: "Progress & accomplishments", ph: "Wins and growth this week.", half: true },
			{ id: "challenges", label: "Challenges", ph: "Obstacles or struggles.", half: true },
			{ id: "guidance", label: "Guidance & feedback", ph: "Advice and direction provided." },
			{ id: "goals", label: "Goals for next week", ph: "What to aim for in the coming week." }
		]
	};
	...
	function isoWeekOf(date) { var d = new Date(Date.UTC(date.getFullYear(), date.getMonth(), date.getDate())); var day = d.getUTCDay() || 7; d.setUTCDate(d.getUTCDate() + 4 - day); var ys = new Date(Date.UTC(d.getUTCFullYear(), 0, 1)); var wk = Math.ceil((((d - ys) / 86400000) + 1) / 7); return d.getUTCFullYear() + "-W" + pad2(wk); }
	function weekStartDate(token) { var m = /^(\d{4})-W(\d{2})$/.exec(token || ""); if (!m) return null; var y = +m[1], w = +m[2]; var simple = new Date(y, 0, 1 + (w - 1) * 7); var dow = simple.getDay(); var monday = new Date(simple); if (dow <= 4) monday.setDate(simple.getDate() - dow + 1); else monday.setDate(simple.getDate() + 8 - dow); return monday; }
	function fmtWeek(token) { var mo = weekStartDate(token); if (!mo) return token ? token : "—"; return "Week of " + mo.toLocaleDateString(undefined, { month: "short", day: "numeric", year: "numeric" }); }
	function fmtWeekLong(token) { var mo = weekStartDate(token); if (!mo) return token ? token : "—"; var end = new Date(mo); end.setDate(mo.getDate() + 6); return "Week of " + mo.toLocaleDateString(undefined, { month: "short", day: "numeric" }) + " – " + end.toLocaleDateString(undefined, { month: "short", day: "numeric", year: "numeric" }); }
	function fmtPeriodShort(v) { return CFG.weekly ? fmtWeek(v) : fmtShort(v); }
	function fmtPeriodLong(v) { return CFG.weekly ? fmtWeekLong(v) : fmtDate(v); }
	function dShort(r) { return fmtPeriodShort(r[CFG.dateField.id]); }
	function dLong(r) { return fmtPeriodLong(r[CFG.dateField.id]); }
	function normalizeRecords() { if (!CFG.weekly) return; DB.records.forEach(function (r) { if (!r[CFG.dateField.id] && r.weekEnding) { if (/^\d{4}-W\d{2}$/.test(r.weekEnding)) r[CFG.dateField.id] = r.weekEnding; else { var d = parseD(r.weekEnding); if (d && !isNaN(d)) r[CFG.dateField.id] = isoWeekOf(d); } } }); }
	...
	function renderOverview() {
		...
		var stats = stat("Entries", rec.length, last ? "Last: " + fmtPeriodShort(last[CFG.dateField.id]) : "None logged yet", "var(--" + CFG.accent + ")") +
			stat("Latest entry", last ? fmtPeriodShort(last[CFG.dateField.id]) : "—", last ? "Most recent record" : "Add an entry", "var(--ink-soft)");
		if (CFG.hasHours) stats += stat("Hours logged", totalHours || "0", totalHours ? "Sum of logged hours" : "Add hours per entry", "var(--men)");
		stats += stat("Skills covered", covered + "/" + skills.length, "Of committed skills", "var(--leaf)");
		var ovHtml = '<div class="stat-grid">' + stats + "</div>" +
			...
			(feed.length ? '<ul class="feed">' + feed.map(function (f) { return '<li><span class="marker ' + CFG.accent + '"></span><div class="fc"><div class="ft">' + f.t + '</div><div class="fd">' + fmtPeriodShort(f.d) + (f.note ? " · " + esc(f.note) : "") + "</div></div></li>"; }).join("") + "</ul>" : '<p class="muted-note">No records logged yet. Add an entry to get started.</p>') +
			"</div></div>";
		if (CFG.hasRating) {
		...
		ovHtml += "</div>" +
			'<div class="panel" style="margin-top:18px"><div class="panel-head"><h3>Skills coverage</h3><span class="eyebrow">' + covered + " of " + skills.length + ' developed</span></div><div class="panel-body"><div class="ratings">' +
	...
	function markInvalid(id, msg) { var g = $("#grp-" + id); g.classList.add("invalid"); var er = $(".err", g); if (er) er.textContent = msg || "This field is required."; var inp = $("#" + id); try { inp.focus(); } catch (e) { } inp.oninput = function () { g.classList.remove("invalid"); if (er) er.textContent = "This field is required."; }; }
	...
	function openRecordModal(id) {
		...
		var inputType = CFG.weekly ? "week" : "date";
		var defPeriod = CFG.weekly ? isoWeekOf(new Date()) : today();
		var body = '<div class="grp two">' + fieldHTML("f_date", CFG.dateField.label, r[CFG.dateField.id] || defPeriod, { type: inputType, req: true }) + fieldHTML("f_auth", CFG.authorField.label, author, { ph: "Name" }) + "</div>";
		for (var i = 0; i < CFG.fields.length; i++) {
			var f = CFG.fields[i], nx = CFG.fields[i + 1];
		...
		openModal({
			kind: CFG.accent, eyebrow: (id ? "Edit · " : "New · ") + CFG.cadenceLabel, title: (id ? "Edit " : "") + CFG.recordNoun, body: body, onSave: function () {
				...
				if (CFG.unique && DB.records.some(function (x) { return x.id !== id && x[CFG.dateField.id] === dv; })) { markInvalid("f_date", "A record for this week already exists."); notify("There is already a record for that week.", "warning"); return false; }
	...
	function rowOf(r) { var row = [CFG.weekly ? fmtPeriodShort(r[CFG.dateField.id]) : r[CFG.dateField.id], r[CFG.authorField.id]]; CFG.fields.forEach(function (f) { row.push(r[f.id] || ""); }); if (CFG.hasRating) row.push(r.rating || ""); if (CFG.hasHours) row.push(r.hours || ""); row.push((r.skills || []).join(", ")); return row; }
	...
	function exportPdf() {
		...
		doc.setFont("helvetica", "normal"); doc.setFontSize(9); doc.setTextColor(120, 125, 132); doc.text("Generated " + today(), M, 68);
	...
	tool.onReady(function (val, fields) {
		...
		try {
			...
			tool.declareParams([
				{ name: "skills", label: "Skills to develop", type: "text", default: DEFAULT_SKILLS.join(", "), hint: "Comma-separated list of skills to track across entries." },
		...
		tool.onValueChange(function (v) { if (v && typeof v === "object") { DB.profile = Object.assign({ employeeName: "", position: "", organization: "", startDate: "", endDate: "", author: "" }, v.profile || {}); DB.records = Array.isArray(v.records) ? v.records : []; normalizeRecords(); syncLinkedName(); renderAll(); updateValidity(); } });
